...
language_base = 'Arabic'
language_others = ['Russian','Korean','Chinese-wechat','Vietnamese','Spanish']
...
for samples in [10000,100000]:
    file_dir = 'path_to_save_{}/grad-mul-param_checkpoint_{}'.format(language_base,samples)
    ...
    for language_other in language_others:
        file_dir = 'path_to_save_{}/grad-mul-param_checkpoint_{}'.format(language_other,samples)
        with tqdm(total=400) as pbar:
            for (name, params) in original_model.named_parameters():
                pbar.update(1)
                if 'layers.' not in name:
                ...
                bottom_k_params_dict[name] = bottom_k_params_dict[name] & ~bool_sensor_min
                


    # 将输出结果保存到CSV文件
    output_file = "single-country-{}-accumulated-grad-mul-param_top-bottom-{}-{}-{}.csv".format(language_base,model_name,k,samples)

    with open(output_file, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Parameter Name", "Parameters Difference top Similarity", "Parameters Difference bottom Similarity"])

        for (name, diff_top),(name2, diff_bottom) in zip(top_k_params_dict.items(),bottom_k_params_dict.items()):
            writer.writerow([name, (diff_top.sum()/diff_top.numel()).item(), (diff_bottom.sum()/diff_bottom.numel()).item()])


    os.makedirs('single-country-{}-accumulated-{}-grad-mul-param/{}/top{}'.format(language_base,model_name,samples,k),exist_ok=True)
    for key,values in top_k_params_dict.items():
        # 定义保存路径
        save_path = os.path.join('single-country-{}-accumulated-{}-grad-mul-param/{}/top{}'.format(language_base,model_name,samples,k),"{}.pt".format(key))

        # 使用torch.save()保存张量到文件
        torch.save(values, save_path)

    os.makedirs('single-country-{}-accumulated-{}-grad-mul-param/{}/bottom{}'.format(language_base,model_name,samples,k),exist_ok=True)
    for key,values in bottom_k_params_dict.items():
        save_path = os.path.join('single-country-{}-accumulated-{}-grad-mul-param/{}/bottom{}'.format(language_base,model_name,samples,k),"{}.pt".format(key))
        torch.save(values, save_path)
