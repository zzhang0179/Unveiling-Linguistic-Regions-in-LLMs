...
"""
由于llama的词表大小为32000，因此存储的时候使用np.uint16存储，一个数字占4个字节，范围在0~2^32-1
    每个sample的长度不固定
    索引需要记录每个句子的起始位置和长度
    句子的起始位置用np.uint64存储，8B，
    长度用np.uint16存储，2B 最大为65536
"""
"""
合并不同的数据集：
...
class DistributedTokenizer:
    ...
    def split(self, lst: List[int]):
        ...
        '''
        原函数会把最后一块长度不足seq_length的引入重复语料，这里做了修改:
        如果这个句子抄过最大长度seq_length 将其丢弃
        '''
        maxlen = self.max_seq_length
        merged_lst = []
        i = j = 0
    ...
    def _re_split(self, src: str, tokenized: List, start_part=False, end_part=False):
        ...
        if len(tokenized) <= self.max_seq_length:
            ...
            if self.args.language.lower() == "english":
                ...
                """需要注意的是，中文好像对于每个句子的开头都会添加一个29871，测试可以参见llama-main/llama/token_sentence.py文件
                   !!!!!但qwen不会有这个现象!!!!!
                """
                new_tokenized = []
                ...
                for i in range(n_block):
                    ...
                    bos = True if i == 0 and start_part == True and self.bos else False  # 是否添加bos
                    eos = True if i == n_block - 1 and end_part == True and self.eos else False  # 是否添加eos
    ...
    def dsmt_encode(self, json_line):
        ...
        if len(sentences) == 1:
            ...
            # _tokenized = self._re_split(sentences[0], _tokenized, start_part=self.bos, end_part=self.eos)
        else:
            _tokenized = []
...
def get_args():
    ...
    parser.add_argument("--language", default="chinese", type=str, help="english, chinese")
    ...
    parser.add_argument("--file_path", default="examples.txt", type=str, help="源文件，每一行都是一个样本")  # 写入
    parser.add_argument("--num_workers", default=1, type=int, help="并行处理数量")  # 写入
    parser.add_argument("--tokenizer_path", default='LLaMA2-Tokenizer',type=str, help="Tokenizer文件")  # 写入
    parser.add_argument("--save_prefix", default="train", type=str, help="保存的时候叫什么,索引文件会添加上.idx,数据文件添加上.bin")  # 写入
    parser.add_argument("--save_path", default="path_to_save/", type=str, help="保存的位置，需要结尾为/")  # 写入
...
def write(args):
    ...
    # encoder_ids = []
    # for line in lines:
    #     input_ids = encoder.dsmt_encode(line)
    #     encoder_ids.append(dict(input_ids=input_ids, raw_input=collate_fn_from_json(line)))
    #     # print(encoder.dsmt_encode(line))
    # with open("example.json","w",encoding='utf-8') as fp:
    #     json.dump(encoder_ids,fp,ensure_ascii=False,indent=4)
    # assert False
    """统计文件行数"""
