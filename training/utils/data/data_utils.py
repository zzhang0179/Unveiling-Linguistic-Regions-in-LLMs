...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    elif "Local/Belle_10W" in dataset_name:
        return raw_datasets.LocalBelle_10W(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/Belle_20W" in dataset_name:
        return raw_datasets.LocalBelle_20W(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/Belle_95W" in dataset_name:
        return raw_datasets.LocalBelle_95W(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/Belle_40W" in dataset_name:
        return raw_datasets.LocalBelle_40W(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/Belle_55W" in dataset_name:
        return raw_datasets.LocalBelle_55W(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/Zhihu_5W" in dataset_name:
        return raw_datasets.LocalZhihu_5W(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/BelleQA_1K" in dataset_name:
        return raw_datasets.LocalBelleQA_1K(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/BelleQA_2K" in dataset_name:
        return raw_datasets.LocalBelleQA_2K(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/BelleQAtr_1K" in dataset_name:
        return raw_datasets.LocalBelleQAtr_1K(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/Belle_1K" in dataset_name:
        return raw_datasets.LocalBelle_1K(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/Belle_2K" in dataset_name:
        return raw_datasets.LocalBelle_2K(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/Belle_5K" in dataset_name:
        return raw_datasets.LocalBelle_5K(output_path, seed, local_rank,
                                                 dataset_name)
    
    elif "Local/Belle_1W" in dataset_name:
        return raw_datasets.LocalBelle_1W(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Local/Belle_5W" in dataset_name:
        return raw_datasets.LocalBelle_5W(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Knowledge" in dataset_name:
        return raw_datasets.Local_Knowledge(output_path, seed, local_rank,
                                                 dataset_name)
    elif "Harmful" in dataset_name:
        return raw_datasets.Local_Harmful(output_path, seed, local_rank,
                                                 dataset_name)
...
class PromptDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.train_phase == 1:
            # 找到第一个 '2' 的索引
            first_2_index = torch.nonzero(self.chosen_dataset[idx]["input_ids"] == 2, as_tuple=False)
            labels = copy.deepcopy(self.chosen_dataset[idx]["input_ids"])
            if len(first_2_index) > 1 :
                labels[first_2_index[0] + 1:] = -100
            
            # # 找到连续出现13, 22550, 29901的位置
            # target_ids = [13, 22550, 29901]
            # # labels_to_iter = labels.cpu().numpy()
            # target_index = len(labels) - len(target_ids) + 1
            # for index in range(len(labels) - len(target_ids) + 1):
            #     # if labels[index:index+len(target_ids)].item() == target_ids:
            #     #     target_index = index
            #     #     break
            #     if labels[index] == 13:
            #         if labels[index+1] == 22550:
            #             if labels[index+2] == 29901:
            #                 target_index = index
            #                 break
            # if target_index <= len(labels) - len(target_ids):
            #     labels[:target_index+3] = -100
            # if len(first_2_index) >= 1 :
            #     labels[first_2_index[0]] = -100

            # # 找到连续出现13, 22550, 29901的位置
            # if len(first_2_index) >= 1:
            #     # print(labels[first_2_index[0] - 1])
            #     # print(labels[first_2_index[0] - 1] == 315 or labels[first_2_index[0] - 1] == 319 or labels[first_2_index[0] - 1] == 350 or labels[first_2_index[0] - 1] == 360)
            #     if labels[first_2_index[0] - 1] == 315 or labels[first_2_index[0] - 1] == 319 or labels[first_2_index[0] - 1] == 350 or labels[first_2_index[0] - 1] == 360:
            #         labels[: first_2_index[0] - 1] = -100
            #     else:
            #         labels[: first_2_index[0]] = -100
            # if len(first_2_index) >= 1 :
    ...
    if local_rank <= 0 and (buf_create_cache.item() != 0 or reload):
        ...
        if train_phase == 1 and sft_only_data_path:
            ...
            if sft_train_datasets:  # Check if sft_train_datasets is not empty
                ...
                if len(data_path) == 0:
                    train_dataset = sft_train_datasets
                else:
